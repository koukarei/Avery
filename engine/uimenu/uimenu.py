...
class UIMenu(Sprite):
    ...
    def __init__(self, player_cursor_interact=True, has_containers=False):
        ...
        """
        from engine.game.game2 import Game
        self.game = Game.game
...
class InputUI(UIMenu):
    def __init__(self, image, pos):
        ...
        self.font = Font(self.ui_font, int(48 * self.screen_scale[1]))
...
class InputBox(UIMenu):
    def __init__(self, pos, width, text="", click_input=False):
        UIMenu.__init__(self, player_cursor_interact=False)
        self._layer = 31
        self.font = Font(self.ui_font, int(30 * self.screen_scale[1]))
        self.pos = pos
...
class TextBox(UIMenu):
    def __init__(self, image, pos, text):
        ...
        self.font = Font(self.ui_font, int(36 * self.screen_scale[1]))
        self.image = image
...
class MenuButton(UIMenu):
    def __init__(self, images, pos, key_name="", font_size=28, layer=1):
        ...
        self.button_over_image = images[0].copy()
        self.button_click_image = images[1].copy()

        self.font = Font(self.ui_font, int(font_size * self.screen_scale[1]))
...
class BrownMenuButton(UIMenu, Containable):  # NOTE: the button is not brown anymore, it is white/yellow
    ...
    def make_buttons(cls, size, text, font):
        ...
        frame = load_image(game.data_dir, (1, 1), "button.png", ("ui"))
        ...
        draw.rect(hover_button, "#003b02", hover_button.get_rect(), 1)
    ...
    def __init__(self, pos, key_name="", width=200, parent=None):
        ...
        self.mouse_over = False
        self.event = False
        self.font = Font(self.ui_font, 17)
        self.text = self.key_name
        self.refresh()
...
class OptionMenuText(UIMenu):
    def __init__(self, pos, text, text_size, button_image=None):
        UIMenu.__init__(self, player_cursor_interact=False)
        self.pos = pos
        self.font = Font(self.ui_font, text_size)
...
class CharacterProfileBox(UIMenu):
    ...
    def __init__(self, pos):
        UIMenu.__init__(self, player_cursor_interact=False)
        self.font = Font(self.ui_font, int(26 * self.screen_scale[1]))
        self.small_font = Font(self.ui_font, int(22 * self.screen_scale[1]))
...
class CharacterInterface(UIMenu):
    ...
    def __init__(self, pos, player, text_popup):
        UIMenu.__init__(self)
        self.header_font = Font(self.ui_font, int(36 * self.screen_scale[1]))
        self.font = Font(self.ui_font, int(22 * self.screen_scale[1]))
        self.small_font = Font(self.ui_font, int(18 * self.screen_scale[1]))
        self.current_row = 0
        self.current_page = 0
        self.max_page = 0
        self.input_delay = 0
...
class ControllerIcon(UIMenu):
    def __init__(self, pos, images, control_type):
        UIMenu.__init__(self)
        self.pos = pos
        self.font = Font(self.ui_font, int(46 * self.screen_scale[1]))
        self.images = images
...
class MapTitle(UIMenu):
    def __init__(self, pos):
        ...
        self.font = Font(self.ui_font, int(44 * self.screen_scale[1]))
        self.pos = pos
        self.name = ""
...
class NameTextBox(UIMenu):
    def __init__(self, box_size, pos, name, text_size=26, layer=15, box_colour=(220, 220, 220),
        ...
        self._layer = layer
        UIMenu.__init__(self)
        self.font = Font(self.ui_font, int(text_size * self.screen_scale[1]))
        self.name = str(name)
...
class NameList(UIMenu):
    def __init__(self, box, pos, name, text_size=26, layer=15):
        self._layer = layer
        UIMenu.__init__(self)
        self.font = Font(self.ui_font, int(self.screen_scale[1] * text_size))
        self.name = str(name)
...
class TextPopup(UIMenu):
    def __init__(self):
        ...
        self.font = Font(self.ui_font, self.font_size)
        self.pos = (0, 0)
